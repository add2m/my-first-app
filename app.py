...
def get_business_status():
    ...
    # يمكنك تعديل المواعيد من هنا (نظام 24 ساعة)
    start_hour = 11  # ميعاد الفتح (11 صباحاً)
    end_hour = 22    # ميعاد القفل (10 مساءً)
...
if current_page == "booking":
    ...
    with st.form("booking_form"):
        ...
        if st.form_submit_button("إرسال البيانات عبر واتساب", use_container_width=True):
            if u_name and u_phone:
                ...
                st.markdown(f'<a href="https://wa.me/{whatsapp_num}?text={msg}" target="_blank" style="background-color: #25D366; color: white; padding: 15px; text-decoration: none; border-radius: 10px; display: block; text-align: center;">اضغطي هنا لتأكيد الحجز</a>', unsafe_allow_html=True)
    ...
    st.info("قائمة الأسعار سيتم تحديثها قريباً.. يمكنك التواصل معنا للاستفسار.")
    if st.button("العودة للرئيسية"):
        st.query_params.clear()
        st.rerun()
    ...
    with st.expander("اضيفي رأيك هنا"):
    ...
    st.write("---")
    with st.expander("🔐 إدارة التعليقات (للمدير)"):
    ...
    # النص الإرشادي للسهم
    st.markdown('''
        <div style="text-align: right; padding-right: 15px; margin-top: -10px; margin-bottom: 10px;">
            <p style="color: #D4AF37; font-size: 13px; font-weight: bold;">
    ...
    for title, p in menu:
        st.markdown(f'<a href="./?p={p}" target="_self" style="text-decoration:none;color:inherit;"><div style="padding:12px; border:1px solid rgba(49, 51, 63, 0.2); border-radius:8px; text-align:center; margin-bottom:12px; background-color: rgba(255, 255, 255, 0.05); font-weight: bold;">{title}</div></a>', unsafe_allow_html=True)
...
with st.sidebar:
    ...
    st.caption("بيوتي سنتر يارا ثروت - جمالك سر ثقتك ✨")
