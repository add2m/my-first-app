...
st.markdown("""
    ...
    <div class="nav-container">
        <a class="nav-link" href="./?p=home" target="_blank">الرئيسية</a>
        <a class="nav-link" href="./?p=booking" target="_blank">الحجز</a>
        <a class="nav-link" href="./?p=prices" target="_blank">الأسعار</a>
        <a class="nav-link" href="./?p=gallery" target="_blank">شغلنا</a>
        <a class="nav-link" href="./?p=reviews" target="_blank">الآراء</a>
    </div>
    ...
    with st.form("booking_form"):
        ...
        if submit_button:
            if u_name and u_phone:
                ...
                st.markdown(f'<a href="https://wa.me/{whatsapp_num}?text={msg}" target="_blank" style="background-color: #25D366; color: white; padding: 15px; text-decoration: none; border-radius: 10px; display: block; text-align: center; font-weight: bold; margin-top:20px;">تأكيد الحجز عبر واتساب</a>', unsafe_allow_html=True)
            else:
                st.error("⚠️ من فضلك ادخلي الاسم ورقم الهاتف على الأقل")
